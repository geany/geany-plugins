...
void main_notebook_destroy(void)
{
	...
	main_notebook = ui_lookup_widget(geany->main_widgets->window, "main_notebook");
	...
	/* fixme */
	if (GTK_IS_WIDGET(doc_nb) && gtk_widget_get_parent(GTK_WIDGET(doc_nb)) == main_notebook)
	{
...
static GtkWidget *create_main_notebook(void)
{
	...
	g_object_set_data(G_OBJECT(main_notebook), "doc_notebook", doc_nb);
