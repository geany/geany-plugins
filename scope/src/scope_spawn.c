/*
 *      scope_spawn.c - this file is part of Geany, a fast and lightweight IDE
 *
...
/* This is a modified version of geany's spawn.c with geany PR #1461
 * applied to it. The reason for this code duplication is to guarantee
 * that the workaround of PR #1461 will not have any side effects on
 * geany itself or users that do not use the scope plugin at all.
 */
...
static gboolean scope_spawn_parse_argv(const gchar *command_line, gint *argcp, gchar ***argvp,
	GError **error)
{
	GError *gerror = NULL;
 ...
 */
static gchar *scope_spawn_get_program_name(const gchar *command_line, GError **error)
{
	gchar *program;
	...
	if (!scope_spawn_parse_argv(command_line, &argc, &argv, error))
		return FALSE;
...
{
	gchar *program = scope_spawn_get_program_name(command_line, error);
...
#ifdef G_OS_WIN32
static gchar *scope_spawn_create_process_with_pipes(wchar_t *w_command_line, const wchar_t *w_working_directory,
...
static void scope_spawn_append_argument(GString *command, const char *text)
{
	const char *s;
 ...
 *  @return @c TRUE on success, @c FALSE on error.
 */
static gboolean scope_spawn_async_with_pipes(const gchar *working_directory, const gchar *command_line,
	...
	gint *stderr_fd, GError **error)
{
	...
	while (argv && *argv)
		scope_spawn_append_argument(command, *argv++);
	...
	if (success)
	{
		...
		failure = scope_spawn_create_process_with_pipes(w_command, w_working_directory,
	...
	if (command_line)
	{
		...
		if (!scope_spawn_parse_argv(command_line, &cl_argc, &cl_argv, error))
			return FALSE;
...
{
	return scope_spawn_async_with_pipes(working_directory, command_line, argv, envp, child_pid,
...
static void scope_spawn_destroy_common(SpawnChannelData *sc)
{
...
static void scope_spawn_timeout_destroy_cb(gpointer data)
{
	SpawnChannelData *sc = (SpawnChannelData *) data;

	scope_spawn_destroy_common(sc);
	g_io_channel_unref(sc->channel);
	sc->channel = NULL;
}


static void scope_spawn_destroy_cb(gpointer data)
{
	SpawnChannelData *sc = (SpawnChannelData *) data;
...
static gboolean scope_spawn_write_cb(GIOChannel *channel, GIOCondition condition, gpointer data)
{
	SpawnChannelData *sc = (SpawnChannelData *) data;
...
static gboolean scope_spawn_read_cb(GIOChannel *channel, GIOCondition condition, gpointer data);

static gboolean scope_spawn_timeout_read_cb(gpointer data)
{
	SpawnChannelData *sc = (SpawnChannelData *) data;

	/* The solution for continuous empty G_IO_IN-s is to generate them outselves. :) */
	return scope_spawn_read_cb(sc->channel, G_IO_IN, data);
}
...
static gboolean scope_spawn_read_cb(GIOChannel *channel, GIOCondition condition, gpointer data)
{
	...
	else if (SPAWN_CHANNEL_GIO_WATCH(sc) && status == G_IO_STATUS_AGAIN)
	{
		...
		if (!SPAWN_CHANNEL_GIO_WATCH(sc))
		{
			...
			g_source_set_callback(new_source, scope_spawn_timeout_read_cb, data, scope_spawn_timeout_destroy_cb);
...
static void scope_spawn_finalize(SpawnWatcherData *sw)
{
	if (sw->exit_cb)
...
static gboolean scope_spawn_timeout_watch_cb(gpointer data)
{
	...
	scope_spawn_finalize(sw);
	return FALSE;
}


static void scope_spawn_watch_cb(GPid pid, gint status, gpointer data)
{
	SpawnWatcherData *sw = (SpawnWatcherData *) data;
	int i;
	...
	for (i = 0; i < 3; i++)
	{
		if (sw->sc[i].channel)
		{
			...
			g_source_set_callback(source, scope_spawn_timeout_watch_cb, data, NULL);
	...
	GChildWatchFunc exit_cb, gpointer exit_data, GPid *child_pid, GError **error)
{
	...
	g_return_val_if_fail(!(spawn_flags & SCOPE_SPAWN_RECURSIVE) || !(spawn_flags & SCOPE_SPAWN_SYNC),
		FALSE);

	if (scope_spawn_async_with_pipes(working_directory, command_line, argv, envp, &pid,
		...
		stderr_cb ? &pipe[2] : NULL, error))
	{
		...
		sw->main_context = spawn_flags & SCOPE_SPAWN_SYNC ? g_main_context_new() : NULL;
		...
		for (i = 0; i < 3; i++)
		{
			...
			if (i == 0)
			{
				...
				callback = (GSourceFunc) scope_spawn_write_cb;
			}
			else
			{
				gboolean line_buffered = !(spawn_flags &
					((SCOPE_SPAWN_STDOUT_UNBUFFERED >> 1) << i));

				condition = G_IO_IN | G_IO_PRI | G_IO_FAILURE;
				callback = (GSourceFunc) scope_spawn_read_cb;
			...
			if (spawn_flags & (SCOPE_SPAWN_STDIN_RECURSIVE << i))
				g_source_set_can_recurse(source, TRUE);
			else if (i)  /* to avoid new string on each call */
				sc->buffer = g_string_sized_new(sc->max_length);

			g_source_set_callback(source, callback, sc, scope_spawn_destroy_cb);
			g_source_attach(source, sw->main_context);
			g_source_unref(source);
		}
		...
		g_source_set_callback(source, (GSourceFunc) scope_spawn_watch_cb, sw, NULL);
		g_source_attach(source, sw->main_context);
		g_source_unref(source);

		if (spawn_flags & SCOPE_SPAWN_SYNC)
		{
...
static void scope_spawn_append_gstring_cb(GString *string, GIOCondition condition, gpointer data)
{
...
static void scope_spawn_get_exit_status_cb(G_GNUC_UNUSED GPid pid, gint status, gpointer exit_status)
{
	...
	gint *exit_status, GError **error)
{
	...
	return scope_spawn_with_callbacks(working_directory, command_line, argv, envp, SCOPE_SPAWN_SYNC |
		SCOPE_SPAWN_UNBUFFERED, stdin_data ? (GIOFunc) scope_spawn_write_data : NULL, stdin_data,
		stdout_data ? scope_spawn_append_gstring_cb : NULL, stdout_data, 0,
		stderr_data ? scope_spawn_append_gstring_cb : NULL, stderr_data, 0,
		exit_status ? scope_spawn_get_exit_status_cb : NULL, exit_status, NULL, error);
}
...
int main(int argc, char **argv)
{
	...
	if (!strcmp(test_type, "syntax") || !strcmp(test_type, "syntexec"))
	{
		...
		while (read_line("command line: ", command_line, sizeof command_line))
		{
			...
			if (scope_spawn_check_command(command_line, argv[1][4] == 'e', &error))
				fputs("valid\n", stderr);
			else
			{
	...
	else if (!strcmp(test_type, "execute"))
	{
		...
		while (read_line("command line: ", command_line, sizeof command_line))
		{
			...
			if (scope_spawn_async_with_pipes(*working_directory ? working_directory : NULL,
	...
	else if (!strcmp(test_type, "redirect") || !strcmp(test_type, "redinput"))
	{
		...
		while (read_line("command line: ", command_line, sizeof command_line))
		{
			...
			ScopeSpawnWriteData stdin_data;
			GError *error = NULL;
			...
			if (!scope_spawn_with_callbacks(NULL, command_line, NULL, NULL, SCOPE_SPAWN_SYNC,
				stdin_text ? (GIOFunc) scope_spawn_write_data : NULL, &stdin_data,
	...
	else if (!strcmp(test_type, "capture"))
	{
		...
		{
			GString *stdin_text = read_string("text to send: ");
			ScopeSpawnWriteData stdin_data = { NULL, 0 };
			...
			if (scope_spawn_sync(NULL, command_line, NULL, NULL, &stdin_data, stdout_data,
