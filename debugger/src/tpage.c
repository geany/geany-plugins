...
static void tpage_create_widgets(void)
{
	...
	scroll = gtk_scrolled_window_new(gtk_scrollable_get_hadjustment(GTK_SCROLLABLE(args_textview)), gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(args_textview)));
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scroll), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
	gtk_container_add(GTK_CONTAINER(scroll), args_textview);
	gtk_container_add(GTK_CONTAINER(args_frame), scroll);

	/* environment */
	env_frame = gtk_frame_new(_("Environment Variables"));
	tree = envtree_init();
	scroll = gtk_scrolled_window_new(gtk_scrollable_get_hadjustment(GTK_SCROLLABLE(tree)), gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(tree)));
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scroll), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
	gtk_container_add(GTK_CONTAINER(scroll), tree);
	gtk_container_add(GTK_CONTAINER(env_frame), scroll);
}
